...
* We only use exceptions for _exceptional_ situations and not for programming control flows, etc. Creating an exception in Java is expensive and hence should not be done for simply testing whether something is present, valid or permitted. In the latter case design your API to return this as a regular result.
...
All this is offered by http://m-m-m.sourceforge.net/apidocs/net/sf/mmm/util/exception/api/package-summary.html#documentation[mmm-util-core], which we propose as a solution.
If you use the https://github.com/devonfw/devon4j/tree/master/modules/rest[devon4j-rest module], this is already included. For Quarkus applications, you need to add the dependency manually.

If you want to avoid additional dependencies, you can implement your own solution for this by creating an abstract exception class `ApplicationBusinessException` extending from `RuntimeException`. For an example of this, see our https://github.com/devonfw-sample/devon4quarkus-reference[Quarkus reference application].
