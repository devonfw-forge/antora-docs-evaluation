...
In case your devon4j server application is not accessed by browsers or the web-client is using link:guide-jwt[JWT based authentication], you are already safe according to CSRF.
...
Devon4ng client configuration for CSRF is described link:https://github.com/devonfw/devon4ng/blob/develop/documentation/guide-consuming-rest-services[here]
