...
To test `devonfw-ide` or for very small projects you can also use these the latest default settings (just hit return when link:setup[setup] is asking for the `Settings URL`).
...
Therefore simply follow the link:usage#admin[admin usage guide].

==  Structure
The settings folder (see `link:variables[SETTINGS_PATH]`) has to follow this file structure:
...
So the `devon` folder contains `devon.properties` files for the link:configuration[configuration] of your environment.
Further, for the IDEs such as link:eclipse[eclipse] or link:vscode[vscode], the according folders contain the templates to manage the workspace via our link:configurator[configurator].
...
* For tools with complex configuration structures like link:eclipse[eclipse], link:intellij.[intellij], or link:vscode[vscode] we provide a smart mechanism via our link:configurator[configurator].
