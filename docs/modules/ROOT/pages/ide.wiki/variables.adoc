...
The `devonfw-ide` defines a set of standard variables to your environment for link:configuration[configuration] via `variables[.bat]` files.
These environment variables are described by the following table.
Those variables printed *bold* are also exported in your shell (except for windows CMD that does not have such concept). Variables with the value `-` are not set by default but may be set via link:configuration[configuration] to override defaults.
...
|`DEVON_IDE_HOME`|e.g. `/projects/my-project`|The top level directory of your `devonfw-ide` link:structure[structure].
...
|`DEVON_IDE_CUSTOM_TOOLS`|`-`|List of custom tools that should be installed additionally. See link:software#custom[software] for further details.
...
|*`WORKSPACE`*|`main`|The link:workspaces[workspace] you are currently in. Defaults to `main` if you are not inside a link:workspaces[workspace]. Never touch this variable in any `variables` file.
|`WORKSPACE_PATH`|`$DEVON_IDE_HOME/workspaces/$WORKSPACE`|Absolute path to current link:workspaces[workspace]. Never touch this variable in any `variables` file.
|*`JAVA_HOME`*|`$DEVON_IDE_HOME/software/java`|Path to JDK
|`SETTINGS_PATH`|`$DEVON_IDE_HOME/settings`|Path to your link:settings[settings]. To keep `oasp4j-ide` legacy behaviour set this to `$DEVON_IDE_HOME/workspaces/main/development/settings`.
...
|`DEVON_SOFTWARE_REPOSITORY`|`-`|Project specific or custom link:software#repository[software-repository].
|`DEVON_SOFTWARE_PATH`|`-`|Globally shared user-specific link:software#shared[local software installation location].
|`ECLIPSE_VMARGS`|`-Xms128M -Xmx768M -XX:MaxPermSize=256M`|JVM options for Eclipse
|deprecated: `ECLIPSE_PLUGINS`|`-`|Array with "feature groups" and "update site URLs" to customize required link:eclipse#plugins[eclipse plugins]. Deprecated - see link:eclipse#plugins[Eclipse plugins].
|`«TOOL»_VERSION`|`-`|The version of the tool `«TOOL»` to install and use (e.g. `ECLIPSE_VERSION` or `MAVEN_VERSION`).
|`EXTRA_JAVA_VERSION`|`-`|An additional (newer) version of link:java[java] that will be used to run java-based IDEs (e.g. link:eclipse[eclipse] or link:intellij[intellij]).
